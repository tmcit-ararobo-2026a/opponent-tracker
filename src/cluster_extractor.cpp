...
ClusterExtractor::ClusterExtractor(const ClusterParams & params)
: params_(params)
{
}
...
std::vector<Cluster> ClusterExtractor::extractClusters(const std::vector<Point3D> & points_in_map)
{
  ...
  // 1. Filter out points that are outside field or inside static obstacles
  ...
  for (size_t i = 0; i < dynamic_filtered_points.size(); ++i)
  {
    ...
    Eigen::Vector3f min_b(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
    Eigen::Vector3f max_b(std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest());
