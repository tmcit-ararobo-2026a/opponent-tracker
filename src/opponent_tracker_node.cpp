    ...
    : rclcpp::Node("opponent_tracker_node", options)
{
    ...
    pub_bucket_ = this->create_publisher<geometry_msgs::msg::PointStamped>(
        "/opponent_robot/bucket_target", 10
    );
    pub_velocity_ =
        this->create_publisher<geometry_msgs::msg::TwistStamped>("/opponent_robot/velocity", 10);
    pub_markers_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/opponent_robot/markers", 10);
    pub_our_robot_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/our_robot/markers", 10);

    RCLCPP_INFO(
        this->get_logger(), "OpponentTrackerNode initialized. Listening to: %s", input_topic.c_str()
    );
}
...
void OpponentTrackerNode::initParameters()
{
    ...
    c_params.cluster_tolerance =
        this->declare_parameter<float>("clustering.cluster_tolerance", 0.35f);
    ...
    c_params.opponent_side   = this->declare_parameter<std::string>("court.opponent_side", "auto");
    c_params.court_y_divider = this->declare_parameter<float>("court.court_y_divider", 0.0f);
    c_params.center_barrier_margin =
        this->declare_parameter<float>("court.center_barrier_margin", 0.30f);
    ...
    t_params.measurement_noise_pos =
        this->declare_parameter<float>("kalman.measurement_noise_pos", 0.02f);
    t_params.measurement_noise_z =
        this->declare_parameter<float>("kalman.measurement_noise_z", 0.02f);
    t_params.max_association_dist =
        this->declare_parameter<float>("kalman.max_association_dist", 2.0f);
...
void OpponentTrackerNode::pointCloudCallback(
...
)
{
        ...
        try {
            ...
            try {
                ...
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000, "TF lookup error: %s", ex2.what()
                );
                return;
            }
        }
    ...
    // 3. Extract candidate clusters (only searching within opponent court)
    const Eigen::Vector3f our_pos_map = transform_to_map.translation();
    std::vector<Cluster> raw_clusters = extractor_.extractClusters(points_in_map, our_pos_map.y());
    std::vector<Cluster> clusters;

    // Filter out clusters too close to our own robot (< 0.9m)
    ...
    if (!clusters.empty()) {
        if (tracker_.isTracking()) {
            ...
            // Pick candidate cluster with highest confidence (bucket presence, point count, shape)
            float max_score = -1.0f;
            int best_idx    = 0;

            for (size_t i = 0; i < clusters.size(); ++i) {
                const auto& c = clusters[i];
                float score   = static_cast<float>(c.points.size());
                if (c.has_bucket) {
                    score += 500.0f;  // Strongly prioritize clusters with detected bucket
                }
                const float vol =
                    c.bbox.dimensions.x() * c.bbox.dimensions.y() * c.bbox.dimensions.z();
                if (vol >= 0.2f && vol <= 2.5f) {
                    score += 200.0f;  // Typical robot volume
                }
                if (score > max_score) {
                    max_score = score;
                    best_idx  = static_cast<int>(i);
    ...
    if (matched) {
        tracker_.update(
            best_cluster.centroid, best_cluster.bucket_center, best_cluster.bbox.dimensions
        );
    } else {
        tracker_.markMissed();
    }
...
void OpponentTrackerNode::publishMarkers(const rclcpp::Time& stamp, const Cluster& detected_cluster)
{
    ...
    char buf[128];
    std::snprintf(
        buf, sizeof(buf), "OPPONENT [TRACKING]\nV=%.2fm/s | Z_b=%.2fm", speed, bucket.z()
    );
