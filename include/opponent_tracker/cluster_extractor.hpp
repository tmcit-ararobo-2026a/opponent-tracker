...
struct ClusterParams
{
  ...
  float field_min_x{0.0f};
  float field_max_x{14.0f};
  float field_min_y{0.0f};
  float field_max_y{13.0f};
};
...
public:
  ...
  std::vector<Cluster> extractClusters(const std::vector<Point3D> & points_in_map);
...
private:
  ...
  {
    return (static_cast<int64_t>(gx) * 73856093) ^
           (static_cast<int64_t>(gy) * 19349663) ^
